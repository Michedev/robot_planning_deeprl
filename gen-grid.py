...
def gen_grid(w: int = 30, h: int = 30, nobstacles: int = 20, x_agent: int = 0, y_agent: int = 0, x_destination: int = 20, y_destination: int= 20, output: str = "grid.txt", set_30x30=False):
    """
    ...
    :param set_30x30: generate a set of 30x30 maps of increasing size of number of blocks
    """
    if set_30x30:
        gen_grids_30x30()
...
def gen_grids_30x30():
    ...
    x0, y0 = 0,0
    maps = 10
    difficulty = [('easy', 0.1), ('medium', 0.3), ('hard', 0.5), ('impossible', 0.7)]
    for d, perc_obs in difficulty:
        for i in range(maps):
            gen_grid(30, 30, int(30 * 30 * perc_obs), x0, y0, randint(15, 29), randint(15, 29), f'maps/grid_{d}_{i}.txt', set_30x30=False)
